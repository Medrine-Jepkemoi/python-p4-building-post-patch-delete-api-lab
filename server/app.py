...
def bakery_by_id(id):
    ...
    if request.method == 'GET':
        ...
        bakery_serialized = bakery.to_dict()
        response = make_response(jsonify(bakery_serialized))
        return response
...
def baked_goods():
    if request.method == 'GET':
        ...
        for baked_good in BakedGood.query.all():
            ...
            baked_good_list.append(baked_goods_dict)

        response = make_response(jsonify(baked_good_list), 200)
        return response
        ...
        response = make_response(jsonify(baked_goods_dict), 201)
        return response


@app.route('/baked_goods/<int:id>', methods = ['GET', 'DELETE'])
def del_baked_goods(id):
